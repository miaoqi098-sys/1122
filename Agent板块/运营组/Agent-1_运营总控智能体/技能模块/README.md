...
本目录是Agent-1唯一正式技能承载目录。知识回答“应该知道什么”，Skill回答“面对任务执行哪套标准能力”，共享工具/数据/运行基础设施负责未来真实读取、调度与执行。
...
- S03→S04、S06→S07关键断点已有正式桥接；
- Task→ExecutionResult→ValidationResult→LearningRecord引用链统一；
- S10正式作为横向StrategyChain稳定器；
- 当前示例已迁移到统一接口；
- 动态运行实现已挂运行依赖，当前不因Skill封板而切换到运行层施工。

> 冻结不等于永不修改。V1.x后续业务语义变化按照`统一接口/V1x变更提案规则.md`处理；拼写、链接、示例/Schema一致性等非破坏性问题可直接维护。

## 十个正式Skill
| 编号 | 正式名称 | Code | 业务规则状态 | 当前接口状态 |
|---|---|---|---|---|
| S01 | 事件校验 | `EventValidation` | V1.x冻结 | 统一scope / normalized_event |
| S02 | 上下文装载 | `ContextLoading` | V1.x冻结 | S01→ContextPackage |
| S03 | 冲突检测 | `ConflictDetection` | V1.x冻结 | ContextPackage→DecisionItemBuilder |
| S04 | 决策排序 | `DecisionPrioritization` | V1.x冻结 | Canonical DecisionItem |
| S05 | 方案生成 | `OptionGeneration` | V1.x冻结 | DecisionItem + ContextPackage |
| S06 | 风险评估 | `RiskAssessment` | V1.x冻结 | ContextPackage→DecisionSelector |
| S07 | 任务编排 | `TaskOrchestration` | V1.x冻结 | FinalDecision→TaskPlan/Task |
| S08 | 结果验证 | `OutcomeValidation` | V1.x冻结 | ExecutionResult→ValidationResult |
| S09 | 经验沉淀 | `LearningWriteback` | V1.x冻结 | ValidationResult→LearningRecord |
| S10 | 策略防抖 | `StrategyStabilization` | V1.x冻结 | StrategyChain/Stabilization |
...
未来Runner禁止把S01-S10实现成简单固定for-loop。

# 统一接口层
`统一接口/`不是新增业务Skill，而是十个Skill之间的公共业务接口层。
...
`run_id / trace_id / parent_run_id`属于Skill Invocation Envelope，不属于业务对象本身。

# 已废弃的新接口别名
新Schema、新示例和未来运行代码不得继续使用：
...
- T01-T80为规则/Schema/示例静态验收；
- T81-T90依赖真实Skill Runner、Schema Validator、Adapter、Scheduler/Executor、Memory Writer。

**当前只确认静态规则层/接口层收口；T81-T90没有真实运行通过。**
...
# 当前建设方向
技能模块本身不再作为Agent-1当前主建设面。其动态实现需求已挂：R01 Skill Runner、R03 Schema Validator、R07/R08调度执行、R09结果标准化、R10学习写入、R11策略链持久化等。

当前仓库建设继续优先完成`Agent库/`各Agent框架与跨Agent接口；只有用户另行启动运行层建设时，才进入Runner/Adapter/执行器等真实实现与T81-T90动态测试。

## 最终状态
**技能模块 V1.x 规则层与统一接口层：阶段性封板；动态运行：未实现/未验收。**