(() => {
  ...
  const tag = value => `<span class="tag tag-${kindFor(value)}">${esc(statusText(value))}</span>`;

  function sourceBar(data, section) {
    const status = data.__source?.source_status || data.source_status || {};
    return `<div class="source-bar"><span class="source-label">边界读模型</span>${tag(status[section] || 'UNKNOWN')}${tag(data.live_data_verified === true ? 'SEMANTIC_VERIFIED' : 'SEMANTIC_PENDING')}<span>非执行参考层</span></div>`;
  }
  ...
  async function renderApr({ data, view, setChrome, isCurrent }) {
    setChrome('市场玩法探索', '情报 / 亚马逊经营边界 / 市场玩法');
    const rows = Array.isArray(data.apr) ? data.apr : [];
    if (!isCurrent()) return;
    view.innerHTML = `
      ${sourceBar(data, 'apr')}
      ${hero('市场玩法探索', '亚马逊真实市场玩法雷达', '记录市场上发生了什么、可能为什么有效、持续多久、如何识别以及对我方的价值。市场玩法不直接产生亚马逊或广告写入权限。')}
      <section class="card section">
        ...
        <label><span class="field-label">关键词</span><input id="apr-filter" class="field" type="search" placeholder="例如评价、流量或排名" /></label>
      </section>
      <section class="section"><div id="apr-table"></div></section>
      <div class="notice warn section">已观察不等于已确认。市场玩法可记录合规、条件允许、未知或非合规市场现象，但永远不直接形成生产执行权。</div>
    `;
    ...
    function draw() {
      ...
      target.innerHTML = `<div class="table-wrap"><table><caption>当前显示 ${visible.length} / ${rows.length} 条市场玩法记录。</caption><thead><tr><th scope="col">编号</th><th scope="col">领域</th><th scope="col">玩法 / 模式</th><th scope="col">经营目标</th><th scope="col">观察效果</th><th scope="col">政策关系</th><th scope="col">经营价值</th><th scope="col">置信度</th></tr></thead><tbody>${visible.length ? visible.map(item => `<tr><td class="code">${esc(item.apr_id)}</td><td>${esc(item.domain)}</td><td><strong>${esc(item.pattern_name_cn)}</strong><div class="item-meta">${esc((item.detection_signals || []).slice(0, 2).join(' · '))}</div></td><td>${esc(item.business_goal)}</td><td>${esc(item.observed_effect)}</td><td>${tag(item.policy_relation)}</td><td>${tag(item.business_value_signal)}</td><td>${tag(item.confidence)}</td></tr>`).join('') : '<tr><td colspan="8">没有匹配结果。</td></tr>'}</tbody></table></div>`;
    }
  ...
  async function renderAom({ data, view, setChrome, isCurrent }) {
    setChrome('正向运营方法', '情报 / 亚马逊经营边界 / 正向运营方法');
    const rows = Array.isArray(data.aom) ? data.aom : [];
    if (!isCurrent()) return;
    view.innerHTML = `
      ${sourceBar(data, 'aom')}
      ${hero('正向运营方法', '从经营目标出发选择正向方法', '每条方法保留目标、条件、预期影响、衡量指标、政策状态与风险。页面提供候选，不自动授权执行。')}
      <section class="card section"><div class="section-head"><div><h2>筛选方法</h2><div class="section-sub">方法名称、目标或衡量指标</div></div><span id="aom-visible" class="tag tag-info">${rows.length} 条</span></div><label><span class="field-label">关键词</span><input id="aom-filter" class="field" type="search" placeholder="例如评价、转化或广告" /></label></section>
  ...
  async function renderApb({ data, view, setChrome, isCurrent }) {
    setChrome('政策与边界证据', '情报 / 亚马逊经营边界 / 政策边界');
    const rows = Array.isArray(data.apb) ? data.apb : [];
    const domains = Array.isArray(data.domains) ? data.domains : [];
    if (!isCurrent()) return;
    view.innerHTML = `
      ${sourceBar(data, 'apb')}
      ${hero('政策边界证据', '政策证据、应有状态与观察状态', '政策边界页面管理官方证据、政策变化、边界信号与商品影响。它负责证据与边界，不是运营方法首页。')}
      <div class="grid grid-4 section">${domains.map(domain => `<article class="card"><div class="code">编号 ${esc(domain[0])}</div><div class="item-title" style="margin-top:7px">${esc(domain[2])}</div><div class="item-meta">${esc(domain[1])}</div><div style="margin-top:10px">${tag(domain[3])}</div></article>`).join('')}</div>
      <section class="section"><div class="section-head"><div><h2>当前政策与边界记录</h2><div class="section-sub">${rows.length} 条</div></div></div><div class="table-wrap"><table><caption>政策边界记录按当前仓库与数据存储读模型展示。</caption><thead><tr><th scope="col">编号</th><th scope="col">领域</th><th scope="col">类型</th><th scope="col">状态</th><th scope="col">标题</th><th scope="col">置信度</th></tr></thead><tbody>${rows.length ? rows.map(item => `<tr><td class="code">${esc(item.case_id)}</td><td>${esc(item.domain)}</td><td>${esc(statusText(item.result_type))}</td><td>${tag(item.status)}</td><td>${esc(item.title_cn)}</td><td>${tag(item.confidence)}</td></tr>`).join('') : '<tr><td colspan="6">当前没有政策边界记录。</td></tr>'}</tbody></table></div></section>
    `;
  }
