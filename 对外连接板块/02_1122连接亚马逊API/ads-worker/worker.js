...
async function adsPost(env, region, path, profileId, mediaType, body, refreshToken) { const auth = await adsAccessToken(env, region, refreshToken); const response = await fetchBounded(`${REGIONS[region].adsApi}${path}`, { method: "POST", headers: { Authorization: `Bearer ${auth.access_token}`, "Amazon-Advertising-API-ClientId": env.AMAZON_ADS_CLIENT_ID, "Amazon-Advertising-API-Scope": String(profileId), Accept: mediaType, "Content-Type": mediaType }, body: JSON.stringify(body) }); let payload; try { payload = await response.json(); } catch { throw new Error(`ADS_NON_JSON_${response.status}`); } if (!response.ok) throw new Error(`ADS_API_${response.status}_${String(payload?.code || "FAILED").slice(0, 48)}`); return payload; }
...
async function campaigns(env, region, profileId, refreshToken) { const payload = await adsPost(env, region, "/sp/campaigns/list", profileId, "application/vnd.spCampaign.v3+json", {}, refreshToken); if (!payload || !Array.isArray(payload.campaigns)) throw new Error("CAMPAIGNS_SCHEMA_INVALID"); return payload.campaigns.map(x => ({ campaignId: String(x.campaignId ?? ""), name: String(x.name ?? ""), state: String(x.state ?? ""), budget: Number.isFinite(Number(x.budget?.budget)) ? Number(x.budget.budget) : null, targetingType: x.targetingType || null })); }
async function adGroups(env, region, profileId, campaignId, refreshToken) { const filters = campaignId ? { campaignIdFilter: { include: [campaignId] } } : {}; const payload = await adsPost(env, region, "/sp/adGroups/list", profileId, "application/vnd.spAdGroup.v3+json", filters, refreshToken); if (!payload || !Array.isArray(payload.adGroups)) throw new Error("AD_GROUPS_SCHEMA_INVALID"); return payload.adGroups.map(x => ({ adGroupId: String(x.adGroupId ?? ""), campaignId: String(x.campaignId ?? ""), name: String(x.name ?? ""), state: String(x.state ?? ""), defaultBid: Number.isFinite(Number(x.defaultBid)) ? Number(x.defaultBid) : null })); }
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/" || url.pathname === "/health") return json({ ok: true, service: "1122-amazon-ads-bridge", status: "online", version: "1.2.0", checked_at: now(), configured: configured(env) }, 200, origin);
    ...
    if (url.pathname === "/oauth/callback") {
      ...
      try { const token = await tokenRequest(region, { grant_type: "authorization_code", code, client_id: env.AMAZON_ADS_CLIENT_ID, client_secret: env.AMAZON_ADS_CLIENT_SECRET, redirect_uri: publicCallbackUrl(env, request) }); if (!token.refresh_token) throw new Error("LWA_REFRESH_TOKEN_MISSING"); const profiles = await listProfiles(env, region, token.refresh_token); if (!profiles.length) throw new Error("PROFILES_EMPTY"); await storeRefreshToken(env, token.refresh_token); return Response.redirect("https://1122.sorilo-uk.com/#/operations/ads", 302); } catch { return error("token_exchange", "TOKEN_EXCHANGE_FAILED", 502, "", "Amazon authorization could not be completed."); }
    }
