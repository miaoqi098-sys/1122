...
function spHeaders(accessToken) {
  return {
    ...
    "user-agent": "1122AmazonBridge/4.5 (Language=JavaScript; Platform=CloudflareWorkers)",
  };
}

async function spRequest(endpoint, path, accessToken, options = {}) {
...
async function spPost(endpoint, path, accessToken, body) {
...
async function coreBatch(env, statements, chunkSize = 50) {
...
function normalizeTrafficDateRecord(row) {
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "POST" && url.pathname === "/internal/migrate-current-data-layer") {
