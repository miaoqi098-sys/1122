import { runAgent2ToS04 } from './a2-a1-s04-runtime.js';
import {
...
export const A2_A1_S04_DECISION_RUNTIME_VERSION = 'A2-A1-S04-decision-runtime-v1.0.0';
...
function failClosed(reasons, s04BridgeResult = null, priorityCandidate = null, priorityResult = null, formationResult = null) {
  return {
    ...
    runtimeVersion: A2_A1_S04_DECISION_RUNTIME_VERSION,
...
/**
 * Agent-2 read-only bridge through S04 priority and decision formation.
 *
 * This runtime intentionally stops at a non-executable decision candidate.
 * It does not generate a TaskDraft, grant approval/permission, dispatch work,
 * write D1, or perform any Amazon/Ads mutation.
 */
export async function runAgent2ToS04DecisionFormation(input, options = {}) {
  ...
  const s04BridgeResult = await runAgent2ToS04(input, options);
  if (
  ...
  if (!isObject(event) || event.source_type !== 'professional_agent' || event.source_agent !== 'Agent-2') {
    return failClosed(['agent2_event_lineage_mismatch'], s04BridgeResult);
  }
  ...
  if (priorityCandidate.decisionItemId !== decisionItem.decision_item_id) {
    return failClosed(['priority_decision_item_lineage_mismatch'], s04BridgeResult, priorityCandidate);
  }
  if (priorityCandidate.eventId !== event.event_id) {
    return failClosed(['priority_event_lineage_mismatch'], s04BridgeResult, priorityCandidate);
  }
  ...
  if (!Array.isArray(priorityResult.rankedDecisionItems) || priorityResult.rankedDecisionItems.length !== 1) {
    return failClosed(
      ['unexpected_ranked_decision_item_count'],
      s04BridgeResult,
      priorityCandidate,
      priorityResult,
    );
  ...
  if (rankedItem.rank !== 1 || rankedItem.decisionItemId !== decisionItem.decision_item_id) {
    return failClosed(
      ['ranked_decision_item_lineage_mismatch'],
      s04BridgeResult,
      priorityCandidate,
      priorityResult,
    );
  ...
  if (!Array.isArray(formationResult.decisionCandidates) || formationResult.decisionCandidates.length !== 1) {
    return failClosed(
      ['unexpected_decision_candidate_count'],
      s04BridgeResult,
      priorityCandidate,
      priorityResult,
      formationResult,
