...
export const A2_A1_S04_TASK_DRAFT_RUNTIME_VERSION = 'A2-A1-S04-task-draft-runtime-v1.0.0';
...
function failClosed(reasons, decisionResult = null, taskDraftResult = null) {
  return {
    ...
    runtimeVersion: A2_A1_S04_TASK_DRAFT_RUNTIME_VERSION,
...
/**
 * Agent-2 read-only bridge from S04 decision formation into TaskDraft generation.
 *
 * This runtime intentionally stops at one draft-only task. It never calls the
 * approval gate, permission decision, dispatch gate, executor or any external
 * service. No DB write or Amazon/Ads mutation is performed here.
 */
export async function runAgent2ToS04TaskDraft(input, options = {}) {
  ...
  const decisionResult = await runAgent2ToS04DecisionFormation(input, options);
  if (
  ...
  if (!isObject(event) || event.source_type !== 'professional_agent' || event.source_agent !== 'Agent-2') {
    return failClosed(['agent2_event_lineage_mismatch'], decisionResult);
  }
