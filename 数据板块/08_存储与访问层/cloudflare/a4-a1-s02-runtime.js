...
export const A4_A1_S02_RUNTIME_VERSION = 'A4-A1-S02-runtime-v1.0.0';
...
function failClosed(reasons, intakeResult = null, s02Result = null) {
  return {
    ...
    runtimeVersion: A4_A1_S02_RUNTIME_VERSION,
...
function validateBoundary(input) {
  ...
  if (input.source_agent !== undefined && input.source_agent !== 'Agent-4') {
    reasons.push('input_source_agent_mismatch');
  }
  return reasons;
}

function validateAgent4Lineage(input, intakeResult) {
  const reasons = [];
  const event = intakeResult?.canonicalEvent;
  const agent4Result = intakeResult?.agent4Result;
  const normalized = agent4Result?.normalizedEvent;
  const domainEvent = agent4Result?.domainEvent;

  if (!isObject(event)) return ['missing_a1_normalized_event'];
  if (!isObject(normalized) || !isObject(normalized.canonicalEvent) || !isObject(domainEvent)) {
    return ['missing_agent4_r16_lineage'];
  }
  ...
  if (!nonEmptyString(productId)
      ...
      || event.scope_id !== input.scope?.scope_id) {
    reasons.push('agent4_product_scope_lineage_mismatch');
  }

  if (event.metrics?.entity_type !== input.entity?.entity_type
  ...
  if (intakeResult.s01Result?.normalized_event?.event_id !== event.event_id
      || intakeResult.s01Result?.normalized_event?.source_agent !== 'Agent-4'
...
/**
 * Read-only Agent-4 -> A1 S02 context bridge.
 * Re-runs Agent-4 -> R16 -> S01, validates advertising entity/window/product lineage,
 * then invokes the shared S02 context loader. Any anomaly fails closed.
 */
export function runAgent4ToS02(input, options = {}) {
  ...
  const intakeResult = runAgent4ToA1Intake(input, {
  ...
  const lineageReasons = validateAgent4Lineage(input, intakeResult);
