...
export const A4_A1_S03_RUNTIME_VERSION = 'A4-A1-S03-runtime-v1.0.0';
...
function failClosed(reasons, s02BridgeResult = null, s03Result = null) {
  return {
    ...
    runtimeVersion: A4_A1_S03_RUNTIME_VERSION,
...
function validateAgent4S03Lineage(input, s02BridgeResult) {
  const reasons = [];
  ...
  if (event.event_type !== 'advertising_conversion_breakdown') {
    reasons.push('agent4_event_type_not_allowed_for_s03');
  }
...
/**
 * Read-only Agent-4 -> A1 S03 bridge.
 * Re-runs Agent-4 -> R16 -> S01 -> S02, validates product/scope, advertising
 * entity/window, event and context lineage, then invokes shared S03 conflict
 * detection. No DecisionItem, approval, dispatch, execution or production write.
 */
export function runAgent4ToS03(input, options = {}) {
  if (!isObject(input)) return failClosed(['invalid_runtime_input']);

  const s02BridgeResult = runAgent4ToS02(input, options);
  ...
  const lineageReasons = validateAgent4S03Lineage(input, s02BridgeResult);
