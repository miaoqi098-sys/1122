...
export const A4_A1_S04_RUNTIME_VERSION = 'A4-A1-S04-runtime-v1.0.0';
...
function failClosed(reasons, decisionBridgeResult = null, s04Result = null) {
  return {
    ...
    runtimeVersion: A4_A1_S04_RUNTIME_VERSION,
...
function validateAgent4S04Lineage(input, decisionBridgeResult) {
  const reasons = [];
  const event = decisionBridgeResult?.canonicalEvent;
  const item = decisionBridgeResult?.decisionItems?.[0];
  const s03Result = decisionBridgeResult?.s03BridgeResult?.s03Result;
  ...
  if (event.event_type !== 'advertising_conversion_breakdown') {
    reasons.push('agent4_event_type_not_allowed_for_S04');
  }
...
/**
 * Read-only Agent-4 bridge into the existing fail-closed S04 ingress/runtime.
 * Re-runs Agent-4 -> R16 -> S01 -> S02 -> S03 -> DecisionItemBuilder,
 * validates source/product/advertising-entity/window/event lineage, and exposes only
 * a synthetic read-only persistence view to S04. No D1 write, approval, permission,
 * execution, dispatch, bid, budget, targeting, campaign-status, or production write occurs.
 */
export async function runAgent4ToS04(input, options = {}) {
  ...
  const decisionBridgeResult = runAgent4ToDecisionItem(input, options);
  ...
  const lineageReasons = validateAgent4S04Lineage(input, decisionBridgeResult);
