...
export const A5_A1_DECISION_ITEM_RUNTIME_VERSION = 'A5-A1-DecisionItem-runtime-v1.0.0';
...
function failClosed(reasons, s03BridgeResult = null, builderResult = null) {
  return {
    ...
    runtimeVersion: A5_A1_DECISION_ITEM_RUNTIME_VERSION,
...
function validateAgent5BuilderLineage(input, s03BridgeResult) {
  const reasons = [];
  ...
  if (event.event_type !== 'traffic_session_decline') {
    reasons.push('agent5_event_type_not_allowed_for_builder');
  }
...
/**
 * Read-only Agent-5 -> DecisionItemBuilder bridge.
 * Re-runs Agent-5 -> R16 -> S01 -> S02 -> S03, validates event/product/
 * traffic-window/context lineage, then builds DecisionItem V1.
 * Stops before S04 and grants no approval, permission, task, dispatch,
 * execution or production-write authority.
 */
export function runAgent5ToDecisionItem(input, options = {}) {
  if (!isObject(input)) return failClosed(['invalid_runtime_input']);

  const s03BridgeResult = runAgent5ToS03(input, options);
  if (s03BridgeResult.status !== 'ready_for_decision_item_builder') {
    return {
      ...failClosed(s03BridgeResult.reasons ?? ['agent5_s03_not_ready'], s03BridgeResult),
  ...
  const lineageReasons = validateAgent5BuilderLineage(input, s03BridgeResult);
