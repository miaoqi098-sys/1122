import { runAgent5ToS04 } from './a5-a1-s04-runtime.js';
import {
...
export const A5_A1_S04_DECISION_RUNTIME_VERSION = 'A5-A1-S04-decision-runtime-v1.0.0';
...
function failClosed(reasons, s04BridgeResult = null, priorityCandidate = null, priorityResult = null, formationResult = null) {
  return {
    ...
    runtimeVersion: A5_A1_S04_DECISION_RUNTIME_VERSION,
...
/**
 * Agent-5 read-only bridge through S04 priority and decision formation.
 * Stops at a non-executable decision candidate. No TaskDraft, approval,
 * permission, dispatch, D1 write, traffic mutation, or production write occurs.
 */
export async function runAgent5ToS04DecisionFormation(input, options = {}) {
  ...
  const s04BridgeResult = await runAgent5ToS04(input, options);
  if (
    s04BridgeResult.status !== 'ready_for_S04_decision_logic' ||
    s04BridgeResult.nextAction !== 'continue_to_S04_decision_logic'
  ) {
    return {
      ...failClosed(s04BridgeResult.reasons ?? ['agent5_s04_not_ready'], s04BridgeResult),
  ...
  if (!isObject(event) || event.source_type !== 'professional_agent' || event.source_agent !== 'Agent-5') {
    return failClosed(['agent5_event_lineage_mismatch'], s04BridgeResult);
  }
  if (event.event_type !== 'traffic_session_decline') {
    return failClosed(['agent5_event_type_not_allowed_for_decision_formation'], s04BridgeResult);
  }
  ...
  if (!expectedProductId || event.product_id !== expectedProductId || event.scope_type !== 'product' || event.scope_id !== expectedScopeId) {
    return failClosed(['agent5_product_scope_lineage_mismatch'], s04BridgeResult);
  }
  if (event.metrics?.product_id !== expectedProductId) {
    return failClosed(['agent5_traffic_product_metric_lineage_mismatch'], s04BridgeResult);
  }
  if (
    ...
    return failClosed(['agent5_traffic_window_lineage_mismatch'], s04BridgeResult);
  }
