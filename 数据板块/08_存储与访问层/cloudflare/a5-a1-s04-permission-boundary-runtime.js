...
export const A5_A1_S04_PERMISSION_BOUNDARY_RUNTIME_VERSION =
  'A5-A1-S04-permission-boundary-runtime-v1.0.0';
...
function failClosed(reasons, approvalGateRuntimeResult = null, permissionDecisionResult = null) {
  return {
    ...
    runtimeVersion: A5_A1_S04_PERMISSION_BOUNDARY_RUNTIME_VERSION,
...
/**
 * Agent-5 read-only bridge from Approval Gate intake to the Permission Decision boundary.
 ...
 */
export async function runAgent5ToS04PermissionBoundary(input, options = {}) {
  ...
  const approvalGateRuntimeResult = await runAgent5ToS04ApprovalGate(input, options);
  if (
  ...
  if (!isObject(event) || event.source_type !== 'professional_agent' || event.source_agent !== 'Agent-5') {
    return failClosed(['agent5_event_lineage_mismatch'], approvalGateRuntimeResult);
  }
  if (event.event_type !== 'traffic_session_decline') {
    return failClosed(['agent5_event_type_not_allowed_for_permission_boundary'], approvalGateRuntimeResult);
  }
  ...
  if (
    ...
    return failClosed(['agent5_product_scope_lineage_mismatch'], approvalGateRuntimeResult);
  }
  if (event.metrics?.product_id !== expectedProductId) {
    return failClosed(['agent5_traffic_product_metric_lineage_mismatch'], approvalGateRuntimeResult);
  }
  if (
    ...
    return failClosed(['agent5_traffic_window_lineage_mismatch'], approvalGateRuntimeResult);
  }
