...
export const A5_A1_S04_RUNTIME_VERSION = 'A5-A1-S04-runtime-v1.0.0';
...
function failClosed(reasons, decisionBridgeResult = null, s04Result = null) {
  return {
    ...
    runtimeVersion: A5_A1_S04_RUNTIME_VERSION,
...
function validateAgent5S04Lineage(input, decisionBridgeResult) {
  const reasons = [];
  const event = decisionBridgeResult?.canonicalEvent;
  const item = decisionBridgeResult?.decisionItems?.[0];
  const s03Result = decisionBridgeResult?.s03BridgeResult?.s03Result;
  ...
  if (event.event_type !== 'traffic_session_decline') {
    reasons.push('agent5_event_type_not_allowed_for_S04');
  }
...
/**
 * Read-only Agent-5 bridge into the existing fail-closed S04 ingress/runtime.
 * Re-runs Agent-5 -> R16 -> S01 -> S02 -> S03 -> DecisionItemBuilder,
 * validates source/product/traffic-window/event lineage, and exposes only a
 * synthetic read-only persistence view to S04. No D1 write, approval,
 * permission, task authorization, execution, dispatch, or production write occurs.
 */
export async function runAgent5ToS04(input, options = {}) {
  ...
  const decisionBridgeResult = runAgent5ToDecisionItem(input, options);
  if (decisionBridgeResult.status !== 'ready_for_S04' || decisionBridgeResult.nextAction !== 'continue_to_S04') {
    return {
      ...failClosed(decisionBridgeResult.reasons ?? ['agent5_decision_item_not_ready'], decisionBridgeResult),
  ...
  const lineageReasons = validateAgent5S04Lineage(input, decisionBridgeResult);
