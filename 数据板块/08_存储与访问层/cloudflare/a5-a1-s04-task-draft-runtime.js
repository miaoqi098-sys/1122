...
export const A5_A1_S04_TASK_DRAFT_RUNTIME_VERSION = 'A5-A1-S04-task-draft-runtime-v1.0.0';
...
function failClosed(reasons, decisionResult = null, taskDraftResult = null) {
  return {
    ...
    runtimeVersion: A5_A1_S04_TASK_DRAFT_RUNTIME_VERSION,
...
/**
 * Agent-5 read-only bridge from verified S04 decision formation to TaskDraft generation.
 * Stops before approval. No DB write, approval grant, permission grant, dispatch,
 * traffic mutation, executor call, Amazon mutation, or production write occurs.
 */
export async function runAgent5ToS04TaskDraft(input, options = {}) {
  ...
  const decisionResult = await runAgent5ToS04DecisionFormation(input, options);
  if (
    decisionResult.status !== 'decision_candidate_ready' ||
    decisionResult.nextAction !== 'continue_to_task_draft_generation'
  ) {
    return {
      ...failClosed(decisionResult.reasons ?? ['agent5_decision_not_ready'], decisionResult),
  ...
  if (!isObject(event) || event.source_type !== 'professional_agent' || event.source_agent !== 'Agent-5') {
    return failClosed(['agent5_event_lineage_mismatch'], decisionResult);
  }
  if (event.event_type !== 'traffic_session_decline') {
    return failClosed(['agent5_event_type_not_allowed_for_task_draft'], decisionResult);
  }
  ...
  if (
    ...
    return failClosed(['agent5_product_scope_lineage_mismatch'], decisionResult);
  }
  if (event.metrics?.product_id !== expectedProductId) {
    return failClosed(['agent5_traffic_product_metric_lineage_mismatch'], decisionResult);
  }
  if (
    ...
    return failClosed(['agent5_traffic_window_lineage_mismatch'], decisionResult);
  }
