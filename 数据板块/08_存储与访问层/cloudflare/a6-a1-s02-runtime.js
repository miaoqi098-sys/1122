...
function failClosed(reasons, intakeResult = null, s02Result = null) {
  return { status:'blocked', nextAction:'hold_for_review', reasons:[...new Set(reasons)], runtimeVersion:A6_A1_S02_RUNTIME_VERSION, s02RuntimeVersion:S02_RUNTIME_VERSION, intakeResult, s02Result, canonicalEvent:null, contextPackage:null, readOnly:true, approvalGranted:false, permissionGranted:false, taskAuthorized:false, executionAuthorized:false, dispatchAuthorized:false, productionWriteAuthorized:false };
}
...
function validateLineage(input, intakeResult) {
  ...
  if (event.event_type !== 'MARGIN_COMPRESSION') reasons.push('a1_event_type_not_allowed');
  if (event.event_id !== normalized.canonicalEvent.event_id || event.event_id !== domainEvent.event_id || event.source_ref !== domainEvent.event_id) reasons.push('agent6_event_identity_lineage_mismatch');
  const productId=input.scope?.product_id ?? null;
  if (!text(productId) || event.product_id !== productId || event.scope_type !== 'product' || event.scope_id !== input.scope?.scope_id) reasons.push('agent6_product_scope_lineage_mismatch');
  ...
  if (intakeResult.s01Result?.normalized_event?.event_id !== event.event_id || intakeResult.s01Result?.normalized_event?.source_agent !== 'Agent-6' || intakeResult.s01Result?.normalized_event?.product_id !== productId) reasons.push('a1_s01_lineage_mismatch');
  return reasons;
}

export function runAgent6ToS02(input, options = {}) {
  ...
  const intakeResult=runAgent6ToA1Intake(input, options);
  if (intakeResult.status !== 'ready_for_S02') return { ...failClosed(intakeResult.reasons ?? ['agent6_intake_not_ready'], intakeResult), status:intakeResult.status, nextAction:intakeResult.nextAction };
  ...
  const s02Result=runS02({
    ...
    s01_validation:{ status:intakeResult.s01Result.status, warnings:intakeResult.s01Result.warnings ?? [], duplicate_signal:intakeResult.s01Result.duplicate_signal ?? {}, validation_summary:intakeResult.s01Result.validation_summary ?? null, validator_version:intakeResult.s01Result.validator_version ?? null },
  ...
  if (s02Result.status === 'blocked') return failClosed(['s02_blocked', ...(s02Result.missing_context ?? []).map((x)=>`missing:${x.domain ?? 'unknown'}`)],intakeResult,s02Result);
  if (s02Result.status === 'needs_information') return { status:'needs_information', nextAction:s02Result.next_action === 'refresh_context' ? 'refresh_context' : 'request_information', reasons:(s02Result.missing_context ?? []).map((x)=>`missing:${x.domain ?? 'unknown'}`), runtimeVersion:A6_A1_S02_RUNTIME_VERSION, s02RuntimeVersion:S02_RUNTIME_VERSION, intakeResult, s02Result, canonicalEvent:event, contextPackage:null, readOnly:true, approvalGranted:false, permissionGranted:false, taskAuthorized:false, executionAuthorized:false, dispatchAuthorized:false, productionWriteAuthorized:false };
  ...
  return { status:'ready_for_S03', nextAction:'continue_to_S03', reasons:[], runtimeVersion:A6_A1_S02_RUNTIME_VERSION, s02RuntimeVersion:S02_RUNTIME_VERSION, plannedDomains, intakeResult, s02Result, canonicalEvent:event, contextPackage:s02Result.context_package, readOnly:true, approvalGranted:false, permissionGranted:false, taskAuthorized:false, executionAuthorized:false, dispatchAuthorized:false, productionWriteAuthorized:false };
}
