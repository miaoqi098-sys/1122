...
export const A7_A1_DECISION_ITEM_RUNTIME_VERSION = 'A7-A1-DecisionItem-runtime-v1.0.0';
...
function failClosed(reasons, s03BridgeResult = null, builderResult = null) {
  return {
    ...
    runtimeVersion:A7_A1_DECISION_ITEM_RUNTIME_VERSION,
    builderRuntimeVersion:DECISION_ITEM_BUILDER_RUNTIME_VERSION,
    s03BridgeResult, builderResult, canonicalEvent:null, contextPackage:null, decisionItems:[],
...
function validateLineage(input, s03BridgeResult) {
  ...
  if (event.event_type !== 'LOW_COVERAGE') reasons.push('agent7_event_type_not_allowed_for_builder');
  const productId = input?.scope?.product_id ?? null;
  const scopeId = input?.scope?.scope_id ?? null;
  if (!nonEmptyString(productId) || event.product_id !== productId || event.scope_type !== 'product' || event.scope_id !== scopeId) reasons.push('agent7_product_scope_lineage_mismatch');
  if (nonEmptyString(input?.scope?.asin) && event.asin !== input.scope.asin) reasons.push('agent7_asin_lineage_mismatch');
  ...
  if (s03Result?.scope?.scope_type !== event.scope_type || (s03Result?.scope?.scope_id ?? null) !== (event.scope_id ?? event.product_id ?? null) || (s03Result?.scope?.product_id ?? null) !== (event.product_id ?? null)) reasons.push('s03_scope_lineage_mismatch');
...
export function runAgent7ToDecisionItem(input, options = {}) {
  ...
  const s03BridgeResult = runAgent7ToS03(input, options);
  if (s03BridgeResult.status !== 'ready_for_decision_item_builder') {
    return { ...failClosed(s03BridgeResult.reasons ?? ['agent7_s03_not_ready'], s03BridgeResult), status:s03BridgeResult.status, nextAction:s03BridgeResult.nextAction, canonicalEvent:s03BridgeResult.canonicalEvent ?? null, contextPackage:s03BridgeResult.contextPackage ?? null };
  }
  if (s03BridgeResult.nextAction !== 'continue_to_decision_item_builder') return failClosed(['s03_bridge_route_not_allowed_for_builder'], s03BridgeResult);
  ...
  if (builderResult.runtime_version !== DECISION_ITEM_BUILDER_RUNTIME_VERSION) return failClosed(['builder_runtime_version_mismatch'], s03BridgeResult, builderResult);
  if (builderResult.next_action !== 'continue_to_S04') {
    return { ...failClosed(builderResult.builder_notes ?? ['builder_not_ready_for_S04'], s03BridgeResult, builderResult), status:builderResult.next_action === 'request_more_context' ? 'needs_information' : 'blocked', nextAction:builderResult.next_action === 'request_more_context' ? 'request_more_context' : 'hold_for_review', canonicalEvent:event, contextPackage };
  }
  if (!Array.isArray(builderResult.decision_items) || builderResult.decision_items.length === 0) return failClosed(['builder_returned_no_decision_items'], s03BridgeResult, builderResult);
  if (builderResult.decision_items.some((item) => !Array.isArray(item.source_event_refs) || !item.source_event_refs.includes(event.event_id))) return failClosed(['decision_item_event_lineage_mismatch'], s03BridgeResult, builderResult);
  return {
    status:'ready_for_S04', nextAction:'continue_to_S04', reasons:[],
    runtimeVersion:A7_A1_DECISION_ITEM_RUNTIME_VERSION,
    builderRuntimeVersion:DECISION_ITEM_BUILDER_RUNTIME_VERSION,
    s03BridgeResult, builderResult, canonicalEvent:event, contextPackage,
