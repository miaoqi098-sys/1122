...
export const PROFESSIONAL_AGENT_FULL_READONLY_CHAIN_VERSION='professional-agent-full-readonly-chain-v1.0.0';
...
export async function runProfessionalAgentFullReadonlyChain(config){
  ...
  if(!text(agentId)||!text(eventType)||!isObject(domainResult)||!isObject(input))return blocked(['invalid_chain_config']);
  ...
  if(!text(event.product_id)||event.scope_type!=='product'||event.product_id!==input.scope?.product_id||event.scope_id!==input.scope?.scope_id)return blocked(['r16_product_scope_lineage_mismatch'],'r16',{domainResult});
  ...
  if(typeof config.validateLineage==='function'){
    const lr=config.validateLineage(event,input,domainResult);
    if(Array.isArray(lr)&&lr.length)return blocked(lr,'domain_lineage',{domainResult});
  }

  const s01=validateS01({event,known_product_ids:Array.isArray(options.knownProductIds)?options.knownProductIds:[event.product_id],existing_events:Array.isArray(options.existingEvents)?options.existingEvents:[],current_time:options.currentTime??event.received_at??event.occurred_at,allowed_sources:['professional_agent',agentId]});
  ...
  const s03=runS03({event_id:event.event_id,scope:{scope_type:event.scope_type,scope_id:event.scope_id??event.product_id,product_id:event.product_id,asin:event.asin??null},context_package:contextPackage,context_refs:contextRefs,normalized_elements:Array.isArray(options.normalizedElements)?options.normalizedElements:undefined,current_time:options.currentTime??event.received_at??event.occurred_at});
  ...
  const builder=runDecisionItemBuilder({scope:{scope_type:event.scope_type,scope_id:event.scope_id??event.product_id,product_id:event.product_id,asin:event.asin??null},source_event_refs:[event.event_id],context_package:contextPackage,context_refs:contextRefs,conflicts:Array.isArray(s03.conflicts)?s03.conflicts:[],conflict_groups:Array.isArray(s03.conflict_groups)?s03.conflict_groups:[],s03_next_action:s03.next_action,current_time:options.currentTime??event.received_at??event.occurred_at});
  ...
  const item=builder.decision_items[0];
  if(!item.source_event_refs?.includes(event.event_id))return blocked(['decision_item_lineage_mismatch'],'DecisionItemBuilder',{domainResult,s01,s02,s03,builder});
  ...
  const pc=buildS04PriorityCandidate(s04);
  if(pc.status!=='candidate_ready'||pc.rankingEligible!==true||pc.nextAction!=='continue_to_priority_policy')return blocked(['priority_candidate_not_ready'],'Priority',{domainResult,s01,s02,s03,builder,s04,pc});
  const pr=rankS04PriorityCandidates([pc]);
  if(pr.status!=='ranking_ready'||pr.rankingEligible!==true||pr.nextAction!=='continue_to_decision_formation'||pr.rankedDecisionItems?.length!==1)return blocked(['priority_policy_not_ready'],'PriorityPolicy',{domainResult,s01,s02,s03,builder,s04,pc,pr});
  const formation=formS04DecisionCandidates(pr);
  if(formation.status!=='formation_ready'||formation.formationEligible!==true||formation.nextAction!=='continue_to_task_draft_generation'||formation.decisionCandidates?.length!==1)return blocked(['decision_formation_not_ready'],'DecisionFormation',{domainResult,s01,s02,s03,builder,s04,pc,pr,formation});
  const candidate=formation.decisionCandidates[0];
  if(candidate.readOnly!==true||candidate.executionAuthorized!==false||candidate.taskGenerationMode!=='draft_only'||candidate.eventId!==event.event_id)return blocked(['decision_candidate_contract_mismatch'],'DecisionFormation',{domainResult,s01,s02,s03,builder,s04,pc,pr,formation});
  const tasks=generateS04TaskDrafts(formation);
  if(tasks.status!=='task_drafts_ready'||tasks.taskDraftEligible!==true||tasks.nextAction!=='continue_to_approval_gate'||tasks.taskDrafts?.length!==1)return blocked(['task_draft_not_ready'],'TaskDraft',{domainResult,s01,s02,s03,builder,s04,pc,pr,formation,tasks});
  const task=tasks.taskDrafts[0];
  if(task.taskState!=='draft'||task.readOnly!==true||task.executionAuthorized!==false||task.approvalRequired!==true||task.dispatchAuthorized!==false||task.eventId!==event.event_id)return blocked(['task_draft_contract_mismatch'],'TaskDraft',{domainResult,s01,s02,s03,builder,s04,pc,pr,formation,tasks});
  const approvals=createS04ApprovalRequests(tasks);
  if(approvals.status!=='approval_pending'||approvals.approvalGateEligible!==true||approvals.nextAction!=='await_human_approval'||approvals.approvalRequests?.length!==1)return blocked(['approval_gate_not_ready'],'ApprovalGate',{domainResult,s01,s02,s03,builder,s04,pc,pr,formation,tasks,approvals});
  const approval=approvals.approvalRequests[0];
  if(approval.approvalStatus!=='pending'||approval.humanApprovalRequired!==true||approval.readOnly!==true||approval.executionAuthorized!==false||approval.permissionGranted!==false)return blocked(['approval_request_contract_mismatch'],'ApprovalGate',{domainResult,s01,s02,s03,builder,s04,pc,pr,formation,tasks,approvals});
  const permission=createS04PermissionDecisions(approvals,[]);
  if(permission.status!=='blocked'||permission.permissionDecisionEligible!==false||permission.nextAction!=='hold_for_review'||!permission.reasons?.includes('approval_record_count_mismatch')||permission.permissionDecisions?.length!==0||permission.executionAuthorized!==false||permission.dispatchAuthorized!==false)return blocked(['permission_boundary_fail_closed_contract_mismatch'],'PermissionBoundary',{domainResult,s01,s02,s03,builder,s04,pc,pr,formation,tasks,approvals,permission});
