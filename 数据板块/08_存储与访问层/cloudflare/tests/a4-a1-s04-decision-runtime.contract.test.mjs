...
function makeInput() {
  return {
    ...
    request_id: 'REQ-A4-S04-DECISION-001',
    scope: {
      scope_type: 'product',
      scope_id: 'PROD-4',
      product_id: 'PROD-4',
...
const ready = await runAgent4ToS04DecisionFormation(
  makeInput(),
...
const missingSourceEvent = await runAgent4ToS04DecisionFormation(
  makeInput(),
...
const forgedPrivilege = await runAgent4ToS04DecisionFormation(
...
const stale = await runAgent4ToS04DecisionFormation(
  staleInput,
  { ...baseOptions, availableContext: context(sourceEvent) },
);
...
console.log('Agent-4 S04 read-only priority and decision formation contract: PASS');
