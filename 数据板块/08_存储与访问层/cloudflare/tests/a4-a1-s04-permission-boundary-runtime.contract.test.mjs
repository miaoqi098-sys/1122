...
import { runAgent4ToS04PermissionBoundary } from '../a4-a1-s04-permission-boundary-runtime.js';
...
function makeInput() {
  return {
    ...
    request_id: 'REQ-A4-S04-PERM-001',
...
const pending = await runAgent4ToS04PermissionBoundary(makeInput(), { ...baseOptions, availableContext: context(sourceEvent) });
...
const missing = await runAgent4ToS04PermissionBoundary(makeInput(), { ...baseOptions, availableContext: context() });
...
for (const key of ['approvalGranted', 'verifiedApprovalRecords', 'permissionGranted', 'taskAuthorized', 'executionAuthorized', 'dispatchAuthorized', 'productionWriteAuthorized']) {
  ...
  const blocked = await runAgent4ToS04PermissionBoundary(
    { ...makeInput(), [key]: value },
...
console.log('Agent-4 S04 permission boundary fail-closed contract: PASS');
