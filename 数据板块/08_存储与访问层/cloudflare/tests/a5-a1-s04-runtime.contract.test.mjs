...
function makeInput() {
  return {
    input_mode:'simulated', request_id:'REQ-A5-S04-001',
...
const ready=await runAgent5ToS04(makeInput(), { ...baseOptions, availableContext:context(sourceEvent) });
...
const missingSourceEvent=await runAgent5ToS04(makeInput(), { ...baseOptions, availableContext:context() });
...
const forgedPrivilege=await runAgent5ToS04(
...
const stale=await runAgent5ToS04(staleInput, { ...baseOptions, availableContext:context(sourceEvent) });
...
const notReady=await runAgent5ToS04(insufficient, { ...baseOptions, availableContext:context(sourceEvent) });
assert.equal(notReady.status,'needs_confirmation');
assert.equal(notReady.nextAction,'observe');
assert.equal(notReady.s04Result,null);
assert.equal(notReady.executionAuthorized,false);
assert.equal(notReady.dispatchAuthorized,false);

console.log('Agent-5 DecisionItem to fail-closed S04 runtime contract: PASS');
