...
function makeInput() {
  return {
    input_mode:'simulated', request_id:'REQ-A5-S04-TASK-DRAFT-001',
...
const ready=await runAgent5ToS04TaskDraft(
  makeInput(),
...
const missing=await runAgent5ToS04TaskDraft(
  makeInput(),
...
const forgedApproval=await runAgent5ToS04TaskDraft(
...
const forgedRecords=await runAgent5ToS04TaskDraft(
...
const stale=await runAgent5ToS04TaskDraft(
  staleInput,
  { ...baseOptions, availableContext:context(sourceEvent) },
);
...
console.log('Agent-5 S04 draft-only task generation contract: PASS');
