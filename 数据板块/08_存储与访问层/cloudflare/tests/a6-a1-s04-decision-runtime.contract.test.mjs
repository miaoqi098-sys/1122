...
function makeInput() {
  return {
    input_mode:'simulated', request_id:'REQ-A6-S04-DECISION-001',
...
const ready=await runAgent6ToS04DecisionFormation(makeInput(), options);
...
assert.equal(ready.formationResult.formationEligible,true);
...
assert.equal(ready.decisionCandidate.priorityRank,1);
...
const missingSourceEvent=await runAgent6ToS04DecisionFormation(makeInput(), { ...baseOptions, availableContext:context() });
...
const forgedPrivilege=await runAgent6ToS04DecisionFormation(
...
const stale=await runAgent6ToS04DecisionFormation(staleInput, options);
...
const basisBlocked=await runAgent6ToS04DecisionFormation(inconsistentCostModel, options);
assert.notEqual(basisBlocked.nextAction,'continue_to_task_draft_generation');
assert.equal(basisBlocked.decisionCandidate,null);
assert.equal(basisBlocked.executionAuthorized,false);
assert.equal(basisBlocked.productionWriteAuthorized,false);

console.log('Agent-6 S04 read-only Decision Formation contract: PASS');
