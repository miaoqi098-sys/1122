...
function makeInput() {
  return {
    input_mode:'simulated', request_id:'REQ-A6-S04-001',
...
const ready=await runAgent6ToS04(makeInput(), options);
...
const missingSourceEvent=await runAgent6ToS04(makeInput(), { ...baseOptions, availableContext:context() });
...
const forgedPrivilege=await runAgent6ToS04(
...
const stale=await runAgent6ToS04(staleInput, options);
...
const basisBlocked=await runAgent6ToS04(inconsistentCostModel, options);
assert.notEqual(basisBlocked.nextAction,'continue_to_S04_decision_logic');
assert.equal(basisBlocked.s04Result,null);
assert.equal(basisBlocked.executionAuthorized,false);
assert.equal(basisBlocked.productionWriteAuthorized,false);

console.log('Agent-6 DecisionItem to fail-closed S04 runtime contract: PASS');
